
{{--Se crea un modal para mostrar en la vista de index.blade.php--}}
@foreach($catalogocuentas as $catalogocuenta)
    <x-adminlte-modal id="modaleditt{{$catalogocuenta->id}}" title="Nueva Cuenta" theme="green" icon="fas fa-plus" >               
        <form method="POST" action="{{ route('catalogocuentas.update', $catalogocuenta->id) }}"  role="form" enctype="multipart/form-data">
            {{ method_field('PATCH') }}
            @csrf
                @include('catalogocuenta.form')
         </form>
    </x-adminlte-modal>
@endforeach